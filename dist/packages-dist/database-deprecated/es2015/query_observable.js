...
export function observeQuery(query, audit = true) {
    ...
    return Observable.create((observer) => {
        let combined = combineLatest(getOrderObservables(query), getStartAtObservable(query), getEndAtObservable(query), getEqualToObservable(query), getLimitToObservables(query));
        if (audit) {
            combined = combined.pipe(auditTime(0));
        }
        combined.subscribe(([orderBy, startAt, endAt, equalTo, limitTo]) => {
            let serializedOrder = {};
...
function mapToLimitToSelection(value, key) {
    if (value instanceof Observable) {
        return value.pipe(map((value) => ({ value, key })));
    }
    else {
        return new Observable(subscriber => {
            subscriber.next({ key, value });
