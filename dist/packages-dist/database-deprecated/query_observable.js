...
export function observeQuery(query, audit) {
    ...
    return Observable.create(function (observer) {
        var combined = combineLatest(getOrderObservables(query), getStartAtObservable(query), getEndAtObservable(query), getEqualToObservable(query), getLimitToObservables(query));
        if (audit) {
...
function mapToOrderBySelection(value, key) {
    if (value instanceof Observable) {
        return value.pipe(map(function (value) {
...
function mapToLimitToSelection(value, key) {
    if (value instanceof Observable) {
        return value.pipe(map(function (value) { return ({ value: value, key: key }); }));
    }
    else {
        return new Observable(function (subscriber) {
            subscriber.next({ key: key, value: value });
