...
function observeQuery(query, audit) {
    ...
    return rxjs.Observable.create(function (observer) {
        var combined = rxjs.combineLatest(getOrderObservables(query), getStartAtObservable(query), getEndAtObservable(query), getEqualToObservable(query), getLimitToObservables(query));
        if (audit) {
            combined = combined.pipe(rxjs_operators.auditTime(0));
        }
...
function mapToOrderBySelection(value, key) {
    if (value instanceof rxjs.Observable) {
        return value.pipe(rxjs_operators.map(function (value) {
...
function mapToLimitToSelection(value, key) {
    if (value instanceof rxjs.Observable) {
        return value.pipe(rxjs_operators.map(function (value) { return ({ value: value, key: key }); }));
    }
    else {
        return new rxjs.Observable(function (subscriber) {
            subscriber.next({ key: key, value: value });
...
function FirebaseListFactory(ref, _a) {
    ...
    return new FirebaseListObservable(ref, function (subscriber) {
        var sub = queryObs
            .pipe(rxjs_operators.map(function (query) {
            var queried = ref;
            ...
            return queried;
        }), rxjs_operators.switchMap(function (queryRef, ix) {
...
function firebaseListObservable(ref, _a) {
    ...
    return listObs.pipe(rxjs_operators.observeOn(new angularfire2.FirebaseZoneScheduler(new _angular_core.NgZone({}), {})));
}
...
function FirebaseObjectFactory(ref, _a) {
    ...
    return objectObservable.pipe(rxjs_operators.observeOn(new angularfire2.FirebaseZoneScheduler(new _angular_core.NgZone({}), {})));
}
